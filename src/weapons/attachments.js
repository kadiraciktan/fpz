...
import { OPTICS, activeOptic } from './defs.js';
 ...
 * Derived from each model's geometry so attachments sit flush:
 *  muzzle/muzzleR — suppressor centre-line and barrel radius at the tip
 *  bodyTop/scopeZ/scopeLen — scope tube height + placement (no scopeLen = no scope)
 *  mag/magSize — extended-magazine box centre + size (null = none)
 *  grip — foregrip box centre under the barrel (null = none)
 *  stock — skeleton stock spec { hide:[parts], from, pad, y } (null = none)
 */
export const ATTACH_ANCHORS = {
  Pistol: {
    muzzle: [0, 0.03, -0.25], muzzleR: 0.016,
    bodyTop: 0.055, scopeZ: -0.05, scopeLen: null,
    mag: [0, -0.14, 0.012], magSize: [0.042, 0.062, 0.042],
    grip: null,
    stock: null,
    sight: { front: [0, 0.045, -0.215], rear: null },
  },
  Rifle: {
    muzzle: [0, 0.03, -0.55], muzzleR: 0.016,
    bodyTop: 0.07, scopeZ: -0.18, scopeLen: 0.22, acog: true,
    mag: [0, -0.038, -0.075], magSize: [0.05, 0.075, 0.09],
    grip: [0, -0.03, -0.35],
    stock: { hide: ['stock', 'buttstock', 'buttplate'], from: 0.1, pad: 0.36, y: 0.025 },
    sight: { front: [0, 0.045, -0.46], rear: [0, 0.07, -0.02] },
  },
  Shotgun: {
    muzzle: [0, 0.03, -0.53], muzzleR: 0.024,
    bodyTop: 0.072, scopeZ: -0.03, scopeLen: null, acog: true,
    mag: [0, -0.038, -0.32], magSize: [0.03, 0.03, 0.36],
    grip: [0, -0.05, -0.45],
    stock: { hide: ['stock', 'buttplate'], from: 0.055, pad: 0.32, y: 0.02 },
    sight: { front: [0, 0.062, -0.45], rear: [0, 0.072, -0.12] },
  },
  Thompson: {
    muzzle: [0, 0.03, -0.22], muzzleR: 0.016,
    bodyTop: 0.07, scopeZ: 0.0, scopeLen: null,
    mag: [0, -0.152, -0.0075], magSize: [0.084, 0.05, 0.09],
    grip: null, // wooden foregrip already part of the model
    stock: { hide: [], from: 0.14, pad: 0.3, y: 0.03 },
    sight: { front: [0, 0.045, -0.19], rear: [0, 0.07, 0.02] },
  },
  M4A1: {
    muzzle: [0, 0.03, -0.52], muzzleR: 0.014,
    bodyTop: 0.062, scopeZ: -0.06, scopeLen: 0.2, acog: true,
    mag: [0, -0.075, -0.06], magSize: [0.045, 0.09, 0.05],
    grip: [0, -0.03, -0.35],
    stock: { hide: ['stock', 'butt'], from: 0.1, pad: 0.34, y: 0.025 },
    sight: { front: [0, 0.055, -0.42], rear: [0, 0.075, -0.02] },
  },
  MP5: {
    muzzle: [0, 0.025, -0.34], muzzleR: 0.011,
    bodyTop: 0.045, scopeZ: -0.03, scopeLen: null,
    mag: [0, -0.085, -0.048], magSize: [0.038, 0.085, 0.045],
    grip: [0, -0.035, -0.22],
    stock: { hide: ['stock', 'butt'], from: 0.09, pad: 0.3, y: 0.02 },
    sight: { front: [0, 0.05, -0.255], rear: [0, 0.05, 0.03] },
  },
  Cal50: {
    muzzle: [0, 0.036, -0.88], muzzleR: 0.021,
    bodyTop: 0.072, scopeZ: -0.1, scopeLen: 0.26, acog: true,
    mag: [0, -0.06, -0.095], magSize: [0.05, 0.07, 0.09],
    grip: [0, -0.04, -0.18],
    stock: { hide: ['stock', 'cheek'], from: 0.16, pad: 0.46, y: 0.03 },
    sight: { front: [0, 0.05, -0.8], rear: [0, 0.06, 0.05] },
  },
  LSW: {
    muzzle: [0, 0.033, -0.58], muzzleR: 0.017,
    bodyTop: 0.055, scopeZ: -0.02, scopeLen: null,
    mag: [0.062, -0.03, -0.045], magSize: [0.075, 0.06, 0.13],
    grip: [0, -0.05, -0.4],
    stock: { hide: ['stock', 'butt'], from: 0.12, pad: 0.38, y: 0.028 },
    sight: { front: [0, 0.058, -0.5], rear: [0, 0.068, 0.02] },
  },
};
...
export function buildAttachmentMeshes(def, att = {}, gunGroup = null) {
  ...
  if (att.extendedMag && A.mag) {
    // Extended magazine box hung below the original mag position.
    const magMat = mat(0x23251f, 0.7, 0.3);
    const mag = new THREE.Mesh(new THREE.BoxGeometry(A.magSize[0], A.magSize[1], A.magSize[2]), magMat);
    mag.position.set(A.mag[0], A.mag[1], A.mag[2]);
    group.add(mag);
  }
  ...
  if (att.lightStock && A.stock) {
    ...
    const len = A.stock.pad - A.stock.from - 0.02;
    const strut = new THREE.Mesh(new THREE.BoxGeometry(0.022, 0.022, len), stockMat);
    strut.position.set(0, A.stock.y, (A.stock.from + A.stock.pad) / 2);
    group.add(strut);
